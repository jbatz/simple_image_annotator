...
def tagger():
    ...
    print(not_end)
    return render_template('grader.html', not_end=not_end, directory=directory, image=image, labels=labels, head=app.config["HEAD"] + 1, len=len(app.config["FILES"]))
...
def main():
    arg_parser = argparse.ArgumentParser(description="extractor")
    arg_parser.add_argument('--image_dir', dest='image_dir', required=True)
    arg_parser.add_argument('--label_dir', dest='label_dir', required=True)
    arg_parser.add_argument('--csv', dest='csv', required=True)
    args = arg_parser.parse_args()

    convert_files(args.in_dir, args.out_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('image_dir', type=str, help='specify the images directory')
    parser.add_argument('mask_dir', type=str, help='specify the mask directory')
    ...
    app.config["LABELS"] = []
    files = None
    for (dir_path, dir_names, mask_files) in walk(app.config["MASK_DIR"]):
        mask_files
        break
    if files == None:
        print("No files")
        exit()
    app.config["MASK_FILES"] = files
    app.config["HEAD"] = 0
    if args.csv:
